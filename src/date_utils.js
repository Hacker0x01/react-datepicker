...
export function getHightLightHolidaysMap(
  highlightDates = [],
  defaultClassName = "react-datepicker__day--highlighted-holiday",
) {
  const dateClasses = new Map();
  for (let i = 0, len = highlightDates.length; i < len; i++) {
    const obj = highlightDates[i];
    if (isDate(obj)) {
      ...
      if (!classNamesArr.includes(defaultClassName)) {
        classNamesArr.push(defaultClassName);
        dateClasses.set(key, { class: classNamesArr, name: "holiday" });
      }
    } else if (typeof obj === "object") {
      const date = obj.date;
      const Name = obj.name;
      const key = formatDate(date, "MM.dd.yyyy");
      const classNamesArr = dateClasses.get(key)?.class || [];
      if (!classNamesArr.includes(defaultClassName)) {
        classNamesArr.push(defaultClassName);
        dateClasses.set(key, { class: classNamesArr, name: Name });
      }
    }
  }
