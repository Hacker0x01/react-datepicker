...
export function getHolidaysMap(
  holidayDates = [],
  defaultClassName = "react-datepicker__day--holiday",
) {
  const dateClasses = new Map();
  for (let i = 0, len = holidayDates.length; i < len; i++) {
    const obj = holidayDates[i].date;
    if (isDate(obj)) {
      const key = formatDate(obj, "MM.dd.yyyy");
      const classNamesArr = dateClasses.get(key) || [];
      if (!classNamesArr.includes(defaultClassName)) {
        classNamesArr.push(defaultClassName);
        dateClasses.set(key, [...classNamesArr, holidayDates[i].name]);
      }
    } else if (typeof obj === "object") {
      const keys = Object.keys(obj);
      const className = keys[0];
...
export function filterArrayByDate(date, dateMap) {
  ...
  return dateMap.get(formattedDate) || [];
}
